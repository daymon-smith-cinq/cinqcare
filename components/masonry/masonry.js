import styles from "./masonry.module.scss";
import buffaloMap from "../../public/masonry--map-buffalo.svg";
import brooklynMap from "../../public/masonry--map-brooklyn.svg";
import Button from "../button/button";

const Masonry = ({ page }) => {
	let content = "";

	// Homepage
	if (page == "home") {
		content = {
			heading:
				"<em>Millions of people</em>, live with chronic and preventable health conditions.",
			paragraph:
				"We are working together with our communities to change that.<br />It's our calling to deliver care where you live.",
			buttonText: "About Us",
			buttonLink: "/about",
			columns: [
				{
					name: "",
					location: "",
					time: "",
					paragraph: "",
					image: "",
				},
			],
		};
	}

	return (
		<section className={styles.home__masonry}>
			<div className={`${styles.masonry__header} ${" wrapper"}`}>
				<header>
					<h2 dangerouslySetInnerHTML={{ __html: content.heading }} />
				</header>
				<div>
					<p dangerouslySetInnerHTML={{ __html: content.paragraph }} />
					<Button link={content.buttonLink} text={content.buttonText} />
				</div>
			</div>
			<div className="wrapper">
				<div className={styles.masonry}>
					<div className={styles.masonry__column}>
						<div
							className={styles.masonry__item}
							style={{ "--item-bg": "url(https://loremflickr.com/500/473)" }}
						>
							<div className={styles.item__content}>
								<div className={styles.item__info}>
									<div className={styles.item__data}>
										<p>
											<strong>Kathy Biddle</strong>
											<br />
											Buffalo, NY
										</p>
										<p>
											<strong>Patient Time</strong>
											<br />3 Years
										</p>
									</div>
									<img src={buffaloMap.src} alt="" />
								</div>
								<p>
									"Thanks to Dr. Bennett and CINQCARE, I’ve regained my strength
									and confidence, thriving in life after chemo."
								</p>
							</div>
							<img src="/images/WomanSun.png" alt="Placeholder" />
						</div>

						<div className={styles.masonry__item}>
							<div className={styles.item__content}>
								<div className={styles.item__info}>
									<div className={styles.item__data}>
										<p>
											<strong>Maria Morales</strong>
											<br />
											Brooklyn, NY
										</p>
										<p>
											<strong>Patient Time</strong>
											<br />2 Years
										</p>
									</div>
									<img src={brooklynMap.src} alt="" />
								</div>
								<p>
									"Dr. Miller and CINQCARE have helped me take control of my
									heart health, giving me the confidence to live my life to the
									fullest."
								</p>
							</div>
							<img src="/images/WomanPort.png" alt="Placeholder" />
						</div>
					</div>

					<div className={styles.masonry__column}>
						<div className={styles.masonry__item}>
							<div className={styles.item__content}>
								<div className={styles.item__info}>
									<div className={styles.item__data}>
										<p>
											<strong>Matthew Trout</strong>
											<br />
											Buffalo, NY
										</p>
										<p>
											<strong>Patient Time</strong>
											<br />
											4.2 Years
										</p>
									</div>
									<img src={buffaloMap.src} alt="" />
								</div>
								<p>
									“CINQCARE has helped me get back into my daily life and daily
									routine. Dr. John has helped managed my condition to excel in
									daily life.”
								</p>
							</div>
							<img src="/images/Wheelchair.png" alt="Placeholder" />
						</div>
					</div>

					<div className={styles.masonry__column}>
						<div className={styles.masonry__item}>
							<div className={styles.item__content}>
								<div className={styles.item__info}>
									<div className={styles.item__data}>
										<p>
											<strong>Charles Kent</strong>
											<br />
											Buffalo, NY
										</p>
										<p>
											<strong>Patient Time</strong>
											<br />
											1.5 Years
										</p>
									</div>
									<img src={buffaloMap.src} alt="" />
								</div>
								<p>
									"Thanks to Dr. Hayes and CINQCARE, my condition is
									well-managed, allowing me to thrive in my daily life."
								</p>
							</div>
							<img src="/images/Hiking.png" alt="Placeholder" />
						</div>
					</div>
				</div>
			</div>
		</section>
	);
};

export default Masonry;
